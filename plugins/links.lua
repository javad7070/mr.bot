local action = function(msg, blocks)
	...
	if blocks[1] == 'link' then
		...
		if not link then
			text = _("*لطفا یک لینک ثبتن کنید* لینکی موجود نیست")
		else
	...
	if blocks[1] == 'setlink' then
		...
		if msg.chat.username then
			...
			if not blocks[2] then
				local text = _("_این دستور صحیح نیست ابتدا دستور /setlink رو تایپ کنید وجلوی اون لینک گروه رو بزارید و ارسال کنید_")
				api.sendReply(msg, text, true)
				return
			end
			--warn if the link has not the right lenght
			if string.len(blocks[2]) ~= 22 and blocks[2] ~= '-' then
				api.sendReply(msg, _("این لینک *معتبر نیست!*"), true)
				return
			end
		...
		if blocks[2] and blocks[2] == '-' then
			db:hdel(hash, key)
			text = _("لینک *تنظیم نشد*")
		else
			local succ = db:hset(hash, key, link)
			local title = msg.chat.title:escape_hard()
			local substitution = '['..title..']('..link..')'
			if succ == false then
				text = _("لینک گروه بروز شد.\n*لینک جدیدگروه*: %s"):format(substitution)
			else
				text = _("لینک گروه تنظیم شد.\n*این لینک*: %s"):format(substitution)
			end
		end
		api.sendReply(msg, text, true)
	end
