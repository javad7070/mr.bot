...
local action = function(msg, blocks)
	...
	if blocks[1] == 'extra' then
	    ...
	    if msg.reply and not blocks[3] then
	    	...
	    	if not file_id then
	    		...
	    		api.sendReply(msg, _("این رسانه ذخیره شد به عنوان پاسخ برای افراد ناآگاه %s"):format(blocks[2]))
	    	end
		else
	    	...
	    	if not res then
	    		if code == 118 then
					api.sendMessage(msg.chat.id, _("این یک متن بزرگ است من نمیتوانم آن را ارسال کنم"))
				else
					local text = _("این متن اشتباه است.\n"
							.. "اگر مشکلی دارید به مدیر ربات پیام دهید [مدیر ربات]"
							.. "(https://telegram.me/cpp_cs).")
					api.sendMessage(msg.chat.id, text, true)
				end
    		else
	    		db:hset(hash, blocks[2], blocks[3])
	    		local msg_id = res.result.message_id
				api.editMessageText(msg.chat.id, msg_id, _("%s دستور ذخیره شد"):format(blocks[2]), false)
    		end
    	end
	elseif blocks[1] == 'extra list' then
	    ...
	    if commands[1] == nil then
	        api.sendReply(msg, _("دستورات تنظیم نشده"))
	    else
	        for k,v in pairs(commands) do
	            text = text..v..'\n'
	        end
	        local out = _("دستورات سفارشی *لیست*:\n") .. text
    ...
    elseif blocks[1] == 'extra del' then
	    ...
	    if success == 1 then
	    	local out = _("%s دستور مورد نظر حذف شد"):format(blocks[2])
	        api.sendReply(msg, out)
	    else
	        local out = _("%s دستور مورد نظر وجود ندارد"):format(blocks[2])
	        api.sendReply(msg, out)
	    end
    else
    	local hash = 'chat:'..msg.chat.id..':extra'
