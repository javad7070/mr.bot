...
local action = function(msg, blocks)
	if msg.chat.type ~= 'private' and not roles.is_admin_cached(msg) then
		if msg.cb then
			api.answerCallbackQuery(msg.cb_id, _("شما مدیر نیستید"))
    	end
    	return
    end
	...
	if blocks[1] == 'lang' and not blocks[2] then
		...
		api.sendKeyboard(msg.chat.id, _("*لیست زبان های موجود*:"), keyboard, true)
	end
	if blocks[1] == 'langselected' and msg.cb then
        ...
        api.editMessageText(msg.chat.id, msg.message_id, _("زبان ربات *فارسی میباشد*"), nil, true)
	end
end
