        ...
        if is_flooding(msg) then
            ...
            if status == 'on' and not msg.cb and not roles.is_admin_cached(msg) then --if the status is on, and the user is not an admin, and the message is not a callback, then:
        	    ...
        	    if res then
        	        ...
        	        if action == 'ban' then
        	            message = _("%s *مسدودشد* برای اسپم!"):format(name)
        	        else
        	            message = _("%s *اخراج شد* برای اسپم!"):format(name)
        	        end
            ...
            if msg.cb then
                api.answerCallbackQuery(msg.cb_id, _("‼️ لطفا از صفحه کلید سوء استفاده نکنید درخواست نادیده گرفته می شود"))
            end
            ...
            if not roles.is_admin_cached(msg) then --ignore admins
    	        ...
    	        if max_reached_var then --max num reached. Kick/ban the user
    	            ...
    	            if res then --kick worked
    	                ...
    	                local message
    	                if status == 'ban' then
			    			message = _("%s *مسدودشد*: برای ارسال رسانه غیرمجاز!\n❗️ `%d` / `%d`"):format(name, n, max)
    	                else
			    			message = _("%s *اخراج شد*: برای ارسال رسانه غیرمجاز!\n❗️ `%d` / `%d`"):format(name, n, max)
    	                end
    	                api.sendMessage(msg.chat.id, message, true)
    	            end
	            else --max num not reached -> warn
			    	local message = _("%s, این نوع از رسانه در این گروه مجاز نمیباشد.\n(%d / %d)"):format(name, n, max)
    ...
    if rtl_status == 'kick' or rtl_status == 'ban' then
        ...
        if check ~= nil and not roles.is_admin_cached(msg) then
    	    ...
    	    if res then
    	        ...
    	        local message = _("%s *اخراج شد*: برای شخصیت راست نویس بودن"):format(name)
    	        if rtl_status == 'ban' then
					message = _("%s *مسدودشد*: برای شخصیت راست نویس بودن"):format(name)
    	        end
    	        api.sendMessage(msg.chat.id, message, true)
    	    end
        end
        ...
        if arab_status == 'kick' or arab_status == 'ban' then
    	    if not roles.is_admin_cached(msg) then
    	        ...
    	        if res then
    	            ...
    	            local message = _("%s *اخراج شد*: برای ارسال پیام عربی"):format(name)
    	            if arab_status == 'ban' then
						message = _("%s *مسدودشد*: برای ارسال پیام عربی"):format(name)
    	            end
    	            api.sendMessage(msg.chat.id, message, true)
    	        end
            end
