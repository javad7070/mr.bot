...
local function doKeyboard_privsett(user_id)
    ...
    local button_names = {
        ['rules_on_join'] = _('قوانین پیوستن'),
        ['reports'] = _('گزارش کاربران')
    }
...
local function action(msg, blocks)
    if not msg.cb then
        if msg.chat.type == 'private' then
            ...
            api.sendKeyboard(msg.from.id, _('تنظیمات خصوصی خود را تغییر دهید'), keyboard, true)
        end
    end
    if msg.cb then
        if blocks[1] == 'alert' then
            api.answerCallbackQuery(msg.cb_id, 'ریپورت', true)
        else
            change_private_setting(msg.from.id, blocks[1])
            local keyboard = doKeyboard_privsett(msg.from.id)
            api.editMarkup(msg.from.id, msg.message_id, keyboard)
            api.answerCallbackQuery(msg.cb_id, _('⚙ تنظیمات تغییریافت'))
        end
    end
end
