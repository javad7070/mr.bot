		    ...
		    if msg.reply and msg.reply.from.id == bot.id then return end
		 	...
		 	if blocks[1] == 'tempban' then
				if not msg.reply then
					api.sendReply(msg, _("کاربر مورد نظر را ریپلای کنید"))
					return
				end
				...
				if not temp then
					if code == 1 then
						api.sendReply(msg, _("برای این کار شما میتوانید به طور مستقیم دستور /banرا ارسال کنید"))
					else
						api.sendReply(msg, _("محدودیت زمانی یک هفته است"))
					end
					return
				end
		    	...
		    	if not res then
					...
					if is_already_tempbanned then
						text = _("زمان بروز رسانی برای %s. مسدود کردن: %s"):format(kicked, time_reply)
					else
						text = _("کاربر %sانقضای مسدودیت.مسدودشد: %s"):format(kicked, time_reply)
		 	...
		 	if blocks[1] == 'kick' then
		    	...
		    	if not res then
		    		...
		    		api.sendMessage(msg.chat.id, _("%s:توسط مدیر\nاخراج شد%s"):format(admin, kicked), true)
		    	end
	    	end
	   		if blocks[1] == 'ban' then
		    	...
		    	if not res then
		    		...
		    		api.sendMessage(msg.chat.id, _("%s:توسط مدیر\nمسدود شد%s"):format(admin, kicked), true)
		    	end
    		end
   			if blocks[1] == 'unban' then
   				api.unbanUser(chat_id, user_id)
   				local text = _("کاربر رفع مسدودیت شد توسط %s!"):format(admin)
